...
interface DocumentWithPageNumber extends Document {
  pageNumber: number;
}

async function handlePageNumber(question: string, documents: DocumentWithPageNumber[]): Promise<string> {
  const pageNumberMatch = question.match(/page (\d+)/i);
  if (!pageNumberMatch) {
    return "I couldn't find a page number in the question.";
  }

  const pageNumber = parseInt(pageNumberMatch[1]);
  const document = documents.find(doc => doc.pageNumber === pageNumber);
  if (!document) {
    return `I couldn't find a document with page number ${pageNumber}.`;
  }

  // Perform your operation here. For example, let's say we want to count the number of words in the document.
  const wordCount = document.content.split(' ').length;

  return `The document on page ${pageNumber} has ${wordCount} words.`;
}
